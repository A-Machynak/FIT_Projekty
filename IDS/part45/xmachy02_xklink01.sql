...
UPDATE tickets SET state='DONE' WHERE id=1;
...
-- Create-send a bug message to module moderator whenever a bug is found
CREATE OR REPLACE TRIGGER createBugMessage
AFTER INSERT
ON module_bug
FOR EACH ROW
DECLARE
    prog_id INTEGER;
BEGIN
    SELECT programmer_id INTO prog_id FROM modules
    WHERE modules.id = :new.module_id;

    INSERT INTO bug_message(created, bug_id, programmer_id)
    VALUES(SYSDATE(), :new.bug_id, prog_id);
END; 
/
-- No, it won't work without this slash

-- CREATE OR REPLACE TRIGGER

-- STORED PROCEDURES x2 --
--CREATE PROCEDURE ...


-- EXPLAIN PLAN --
-- How many users are using specific programming languages
EXPLAIN PLAN 
        INTO plan_table
    FOR
    SELECT modules.name, COUNT(software_language.id) AS "Programming languages used"
...
-- How many programming languages does each module use
DROP MATERIALIZED VIEW module_langs;
CREATE MATERIALIZED VIEW module_langs AS
    SELECT modules.name, COUNT(software_language.id) AS "Programming languages used"
    FROM modules
        RIGHT JOIN module_language
        ON modules.id = module_language.module_id
...
-- ACCESS CONTROL
GRANT ALL ON ticket_patch TO xklink01;
GRANT ALL ON bug_message TO xklink01;
GRANT ALL ON patch_module TO xklink01;
GRANT ALL ON module_bug TO xklink01;
GRANT ALL ON ticket_bug TO xklink01;
GRANT ALL ON vulnerability TO xklink01;
GRANT ALL ON bugs TO xklink01;
GRANT ALL ON module_language TO xklink01;
GRANT ALL ON modules TO xklink01;
GRANT ALL ON tickets TO xklink01;
GRANT ALL ON patches TO xklink01;
GRANT ALL ON user_language TO xklink01;
GRANT ALL ON programmers TO xklink01;
GRANT ALL ON users TO xklink01;
GRANT ALL ON software_language TO xklink01;
